    ...
    \MageModule\OrderImportExport\Model\Processor\ProcessorInterface
{
    ...
    private function prepareData(\Magento\Sales\Api\Data\OrderInterface &$order)
    {
        ...
        if (!$order->getBaseToGlobalRate()) {
            $rate = 1;
            /** searching for some way to calculate the base_to_global_rate */
            if ($order->getBaseGrandTotal() && $order->getGrandTotal()) {
                $rate = $order->getGrandTotal() / $order->getBaseGrandTotal();
            } elseif ($order->getBaseSubtotal() && $order->getSubtotal()) {
                $rate = $order->getSubtotal() / $order->getBaseSubtotal();
            }

            $order->setBaseToGlobalRate($rate);
        }
