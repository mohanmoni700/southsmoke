...
namespace HookahShisha\Customerb2b\Plugin\Company\Customer\Api;
 ...
 */
class AccountManagement extends \Magento\Company\Plugin\Customer\Api\AccountManagement
{
    /**
    ...
    public function afterCreateAccount(
        ...
        /* if we remove from here it is sending email to the customer to created the approved status company account */
        ...
        if (is_array($companyData) && !empty($companyData)) {
            ...
            /* Start Removed this becuase we need to set as approved company and customer able to login */
            $companyDataObject->setStatus(1);
            $companyDataObject->save();
            /* End */
